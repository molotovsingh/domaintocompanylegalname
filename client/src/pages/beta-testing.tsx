                      ...
                      {result.method === 'perplexity_llm' && result.llmResponse?.parsedJson ? (
                        <div className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                              <span className="text-sm font-medium text-muted-foreground">Processing Confidence:</span>
                          ...
                          {result.llmResponse.parsedJson.sources && result.llmResponse.parsedJson.sources.length > 0 && (
                            <div>
                              <span className="text-sm font-medium text-muted-foreground">Sources Found:</span>
                              <div className="text-sm mt-2 space-y-1 bg-gray-50 p-3 rounded">
                                {result.llmResponse.parsedJson.sources.slice(0, 3).map((source: string, idx: number) => (
                                  <div key={idx} className="text-xs text-muted-foreground break-words">• {source}</div>
                                ))}
                                {result.llmResponse.parsedJson.sources.length > 3 && (
                                  <div className="text-xs text-muted-foreground italic">... and {result.llmResponse.parsedJson.sources.length - 3} more sources</div>
                                )}
                              </div>
                            </div>
                          )}
