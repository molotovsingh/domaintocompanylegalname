    ...
    try {
      ...
      try {
        ...
        if (userBias.muteRankingRules) {
          console.log('[Perplexity Arbitration] Ranking rules MUTED - using LLM ranking as-is');
          rankedEntities = mergedRankedEntities;
        } else {
          console.log('[Perplexity Arbitration] Applying user bias to ranking');
          rankedEntities = this.applyUserBiasToRanking(mergedRankedEntities, userBias);
        }
      ...
      // Return fallback result with first claim as winner
      const fallbackEntity: RankedEntity = {
        rank: 1,
        claimNumber: claims[0]?.claimNumber || 0,
        entityName: claims[0]?.entityName || 'Unknown',
        leiCode: claims[0]?.leiCode || '',
        confidence: 0.5,
        reasoning: 'Arbitration failed - using first claim as fallback',
        acquisitionGrade: 'C',
        metadata: claims[0]?.metadata
      };
...
Key Factor: ${rankedEntities[0]?.metadata?.hierarchyLevel === 'ultimate_parent' ? 'Ultimate parent with full control' : 
  ...
  private applyUserBiasToRanking(entities: RankedEntity[], userBias: UserBias): RankedEntity[] {
    ...
    const scoredEntities = entities.map(entity => {
      ...
      if (userBias.preferParent) {
        if (metadata.hierarchyLevel === 'ultimate_parent') {
          biasScore += userBias.parentWeight;
        } else if (metadata.hierarchyLevel === 'parent') {
          biasScore += userBias.parentWeight * 0.7; // Direct parents get a slightly lower bonus
        }
        // Subsidiaries and independent entities receive no parent bonus
      }

      // Prioritize active entities
      if (metadata.entityStatus === 'ACTIVE') {
        biasScore += userBias.entityStatusWeight;
      }

      // Prefer certain legal forms (e.g., corporations)
      const legalForm = metadata.legalForm || '';
      if (['XTIQ', 'XDLC', '8888'].includes(legalForm)) { // Common corporation codes
        biasScore += userBias.legalFormWeight;
      }

      // Prefer more recent data
      if (metadata.lastUpdateDate) {
        const monthsAgo = (Date.now() - new Date(metadata.lastUpdateDate).getTime()) / (1000 * 60 * 60 * 24 * 30);
        if (monthsAgo < 12) {
          biasScore += userBias.recencyWeight;
        } else if (monthsAgo < 36) {
          biasScore += userBias.recencyWeight * 0.5; // Moderate recency bonus
        }
      }

      // Industry focus bias (if applicable)
      if (userBias.industryFocus && metadata.industryCode) {
        if (userBias.industryFocus.target.includes(metadata.industryCode)) {
          biasScore += 0.2; // Small bonus for target industry
        }
        if (userBias.industryFocus.exclude.includes(metadata.industryCode)) {
          biasScore -= 0.5; // Penalty for excluded industry
        }
      }

      // Combine LLM confidence with bias score
      // We can adjust the weighting here, e.g., give more weight to LLM confidence initially
      const finalScore = (entity.confidence * 0.7) + (biasScore * 0.3);

      return {
        ...entity,
        confidence: Math.min(finalScore, 1.0), // Cap confidence at 1.0
        reasoning: `${entity.reasoning}. Bias Score: ${biasScore.toFixed(2)}`
      };
    });

    // Re-rank entities based on the combined score
    scoredEntities.sort((a, b) => b.confidence - a.confidence);

    // Update ranks and return
    return scoredEntities.map((entity, index) => ({
      ...entity,
      rank: index + 1
    }));
  }
