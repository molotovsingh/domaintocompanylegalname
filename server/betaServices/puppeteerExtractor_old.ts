      ...
      const extractedData = await page.evaluate(() => {
        ...
        const findSocialMedia = () => {
          ...
          for (const link of links) {
            ...
            for (const [platform, pattern] of patternEntries) {
        ...
        const extractContactInfo = () => {
          ...
          const emails = Array.from(new Set(emailMatches)).filter(email => 
        ...
        const getPageMetadata = () => {
          ...
          return {
            ...
            legal: { termsUrl: null, privacyUrl: null, legalUrls: [] },
            aboutUrl: null,
            socialMedia: {},
            contactInfo: { emails: [], phones: [], addresses: [], hasContactPage: false },
            pageMetadata: { title: document.title || '', description: null, keywords: null, viewport: null, charset: document.characterSet || '', htmlLang: document.documentElement.lang || '' },
            htmlSize: 0,
            error: error.message
          };
        }
