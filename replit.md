...
- **Core Logic**:
    ...
    - **MCP Arbitration Framework V1 (Updated Aug 16, 2025)**: Claims-to-arbitration system now using Perplexity API with sonar-pro model for transparent, judge-like entity ranking. Claim 0 represents LLM-extracted entity from website, Claims 1-N are GLEIF candidates. Perplexity provides comprehensive analysis with web search capabilities, incorporating real-time corporate data and citations. Arbitration applies multi-tier ranking: parent entities > subsidiaries, entity status, legal form, and registration recency. **Jurisdiction Discovery Update (Aug 16, 2025)**: Removed preset jurisdiction preferences entirely - system now discovers jurisdiction from domain data (TLD, entity metadata, headquarters location) rather than using user biases. Ranking weights redistributed: Parent (50%), Entity Status (20%), Legal Form (15%), Recency (15%). Designed for acquisition research with emphasis on ultimate parent identification and transparent decision-making. Full implementation plan documented in `docs/MCP-Arbitration-Framework-V1.md`. Frontend integration complete with ArbitrationResults and UserBiasConfig components fully integrated into Beta V2 dashboard, supporting real-time arbitration processing with Perplexity reasoning and user preference management. Fixed critical database schema issues where queries referenced non-existent 'pages' column instead of 'dump_data' JSONB field. Entity extraction now correctly prioritizes og:site_name meta tag over page titles preventing incorrect entity searches. Arbitration now accepts existing GLEIF claims from frontend to avoid regenerating with wrong entity names. **Perplexity Fallback Search (Aug 16, 2025)**: Added PerplexityEntitySearchService as fallback when GLEIF returns no matches, ensuring multiple claims for arbitration. Uses correct "sonar" model (not outdated model names). Creates Claim 1 when GLEIF fails to prevent single-claim arbitrations. Fixed axios_cheerio_dumps database query to properly handle table structure without dump_data column. **UI Cleanup (Aug 16, 2025)**: Major interface streamlining - removed redundant Arbitration tab from BetaV2DataProcessing page, consolidated all LLM model selection into single Global LLM Model Selection card, and completely eliminated duplicate LLM Cleaning section from beta-testing-v2 page. All AI-powered processing is now centralized in Data Processing Stage 2 page with unified model configuration, eliminating confusion from multiple model selection interfaces.
