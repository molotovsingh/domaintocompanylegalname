...
- **Core Logic**:
    ...
    - **Database Large Payload Fix (Complete as of Aug 13, 2025)**: Fixed PostgreSQL JSONB casting issue that prevented large crawl data (1-2MB+) from being saved. Changed from explicit text-to-jsonb casting (`$1::text::jsonb`) to direct object passing, allowing the PostgreSQL driver to handle JSONB conversion automatically. This resolved the "could not determine data type of parameter $1" error that was causing fnp.com and other large site crawls to fail.
